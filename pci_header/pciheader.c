// Code Revised by: Jan Lendl R. Uy
// Expected Input: <bus_num> <slot_num> <function_num>
// PCIe device details can be found using lspci command
...
void print_pci_header(struct pci_dev *pdev) {
	...
	for(i=0; i<0x40; i+=4){
		...
		while(ptr[bitfield].offset < i+4){
			...
			int byte_size = (strlen(str_value)-2)/2;
			int end_addr = start_addr+byte_size-1;
			if (byte_size == 1) {
				printf("%s\n    Address: 0x%X\n", str_value, start_addr);
			} else {
				printf("%s\n    Address Range:0x%X-0x%X\n", str_value, start_addr, end_addr);
			}
			start_addr += byte_size;
			bitfield++;
		}
